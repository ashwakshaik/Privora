...
import { ArrowRight, X, ShieldAlert, AlertTriangle, Info } from "lucide-react";
...
export function RemovalCenter() {
  ...
  useEffect(() => {
    ...
    const loadRemovalData = async () => {
      try {
        ...
        const rawRemovals = await storage.getRemovalRequests(user.id);
  ...
  const handleAutopilotToggle = async () => {
    ...
    setAutopilot(newVal);
    try {
      await storage.saveSettings(user.id, { autopilot_enabled: newVal });
  ...
  return (
    <div className="flex gap-6 relative min-h-[calc(100vh-120px)]">
      ...
      <div className="flex-1 space-y-8 min-w-0 text-left">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          <div className="flex items-center space-x-3 bg-card border border-border px-4 py-2 rounded-xl shadow-premium">
            ...
            <button
              ...
              aria-label="Toggle Autopilot continuous sweeps"
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4" aria-label="Removal Statistics">
        ...
        {/* Filter Tab bar */}
        <div className="flex space-x-1 border-b border-border" role="tablist" aria-label="Filter Removals">
                  ...
                  <TableRow key={idx} className="border-b border-border hover:bg-muted/10">
                    ...
                    <TableCell className="text-right pr-6">
                      <Button
                        ...
                        className="h-8 text-xs text-primary hover:text-primary hover:bg-primary/5 rounded-lg focus-visible:ring-2 focus-visible:ring-primary"
                        aria-label={`View opt-out logs for ${res.broker}`}
                      >
                        Details
      ...
      {selectedBroker && (
        <aside 
          className="w-80 border-l border-border/65 bg-card p-6 flex flex-col justify-between absolute md:sticky top-0 right-0 h-[calc(100vh-120px)] z-15 shadow-2xl rounded-r-[22px] text-left animate-in slide-in-from-right duration-200"
          role="complementary"
          aria-label={`Removal trace details for ${selectedBroker.broker}`}
        >
          ...
          <div className="border-t border-border pt-4">
            <Button
              onClick={() => setSelectedBroker(null)}
              variant="outline"
              className="w-full h-9 text-xs rounded-xl border-border text-foreground hover:bg-muted/50 focus-visible:ring-2 focus-visible:ring-primary"
            >
              Close Drawer
            </Button>
          </div>
