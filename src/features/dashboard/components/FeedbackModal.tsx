...
export function FeedbackModal() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await storage.submitFeedback(user.id, category, rating, message);
  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed bottom-6 right-6 z-45 bg-gradient-to-r from-primary to-secondary text-white px-4 py-2.5 rounded-full flex items-center space-x-2 shadow-premium hover:opacity-90 active:scale-95 transition-all duration-150 cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2"
          ...
          <div className="bg-[#09090B] border border-border/80 rounded-2xl w-full max-w-md p-6 relative shadow-premium overflow-hidden">
            ...
            <div className="flex items-center justify-between mb-4 relative">
              <div className="flex items-center space-x-2">
                ...
                <h3 id="feedback-title" className="text-sm font-bold text-foreground font-heading">
            ...
            <form onSubmit={handleSubmit} className="space-y-4 relative text-left">
                ...
                <div className="grid grid-cols-3 gap-2" role="radiogroup" aria-label="Feedback Category">
                  {(["bug", "suggestion", "other"] as const).map((cat) => (
                    <button
                      ...
                      className={`py-1.5 rounded-lg text-xs font-semibold border capitalize transition-all duration-150 cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary ${
                        category === cat
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex items-center space-x-1.5" role="group" aria-label="Experience Rating Stars">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <button
                      key={star}
                      type="button"
                      onClick={() => setRating(star)}
                      className="text-muted-foreground hover:scale-110 active:scale-95 transition duration-100 cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 rounded"
              ...
              <div className="space-y-1.5">
                ...
                <textarea
                  ...
                  rows={4}
                  className="w-full bg-muted/20 border border-border rounded-xl p-3 text-xs text-foreground placeholder-muted-foreground focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary resize-none"
                />
              </div>
              ...
              <Button
                ...
                className="w-full h-10 rounded-xl bg-gradient-to-r from-primary to-secondary text-white font-semibold flex items-center justify-center border-0 cursor-pointer focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2"
              >
                {isSubmitting ? (
                  <>
