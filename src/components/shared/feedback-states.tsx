...
export function LoadingSpinner({ className, size = 32, label }: LoadingSpinnerProps) {
  return (
    <div className={cn("flex flex-col items-center justify-center p-8", className)}>
      ...
      >
        <Loader2 className="text-primary animate-pulse" size={size} />
      </motion.div>
...
export function ErrorState({
  ...
  return (
    ...
    >
      ...
      {onRetry && (
        <Button onClick={onRetry} variant="secondary" className="mt-5 border-destructive/20 hover:bg-destructive/5 text-destructive" size="sm">
          Retry Action
        </Button>
      )}
    </div>
