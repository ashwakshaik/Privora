...
const WelcomeTour = dynamic(() => import("@/components/shared/welcome-tour").then(m => m.WelcomeTour), { ssr: false });
const FeedbackModal = dynamic(() => import("@/components/shared/feedback-modal").then(m => m.FeedbackModal), { ssr: false });
...
export function DashboardShell({ children }: DashboardShellProps) {
  ...
  return (
    <div className="min-h-screen flex bg-background text-foreground relative overflow-hidden bg-aurora">
      <WelcomeTour />
      <FeedbackModal />
      {/* 1. Desktop Sidebar */}
      <Sidebar
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden fixed inset-0 z-30 bg-background/80 backdrop-blur-sm">
            ...
            <Button
              ...
              className="absolute top-4 right-4 h-9 w-9 z-40 text-muted-foreground hover:text-foreground"
